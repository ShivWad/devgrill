...
import { TECH_LOADING_MSGS, TECH_GLOBAL_STYLES } from "./types";

interface LoadingViewProps {
  resuming?: boolean;
}

export function TechLoadingView({ resuming = false }: LoadingViewProps) {
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      setVisible(false);
      fadeOut = setTimeout(() => {
        setIdx((i) => (i + 1) % TECH_LOADING_MSGS.length);
        setVisible(true);
      }, 350);
    }, 3000);
    return () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        alignItems: "center", justifyContent: "center", gap: 28, padding: "40px 24px",
      }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 360 }}>
          {resuming ? (
            <>
              <p style={{ fontSize: 16, fontWeight: 500, color: "var(--fg-2)", lineHeight: 1.5 }}>Calling Mr. Grill back…</p>
              <p style={{ fontSize: 13, color: "var(--fg-dim)", marginTop: 8 }}>Restoring your session.</p>
            </>
          ) : (
            <>
              <p style={{
