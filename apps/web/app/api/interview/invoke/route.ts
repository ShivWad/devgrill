...
export async function POST(req: NextRequest) {
  ...
  if (userId && getToken) {
    ...
    log.info('Starting interview', { userId })
    ...
    try {
      ...
      if (!res.ok) log.warn('Agent error on invoke', { userId, status: res.status })
      else log.info('Interview invoked', { userId })
      return NextResponse.json(data, { status: res.status })
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err))
      log.error('Agent proxy failed on invoke', { userId, err: error.message, stack: error.stack })
  ...
  log.info('Starting guest interview', { guestId })
  ...
  try {
    ...
    const data = await res.json()
    if (!res.ok) {
      log.warn('Agent error on guest invoke', { guestId, status: res.status })
      return NextResponse.json(data, { status: res.status })
    }

    log.info('Guest interview invoked', { guestId })
    ...
    const error = err instanceof Error ? err : new Error(String(err))
    log.error('Agent proxy failed on guest invoke', { guestId, err: error.message, stack: error.stack })
    return NextResponse.json({ error: 'Failed to reach interview service' }, { status: 502 })
  }
}
