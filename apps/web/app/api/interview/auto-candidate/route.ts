...
export async function POST(req: NextRequest) {
  ...
  if (userId) {
    ...
    try {
      ...
      if (!res.ok) log.warn('Agent error on auto-candidate', { userId, threadId, status: res.status })
      return NextResponse.json(data, { status: res.status })
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err))
      log.error('Agent proxy failed on auto-candidate', { userId, threadId, err: error.message, stack: error.stack })
  ...
  log.debug('Requesting guest auto-candidate answer', { guestId, threadId })
  ...
  try {
    ...
    const data = await res.json()
    if (!res.ok) log.warn('Agent error on guest auto-candidate', { guestId, threadId, status: res.status })
    return NextResponse.json(data, { status: res.status })
  } catch (err) {
    const error = err instanceof Error ? err : new Error(String(err))
    log.error('Agent proxy failed on guest auto-candidate', { guestId, threadId, err: error.message, stack: error.stack })
    return NextResponse.json({ error: 'Failed to reach interview service' }, { status: 502 })
  }
}
